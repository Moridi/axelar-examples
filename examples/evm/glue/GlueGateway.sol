// SPDX-License-Identifier: MIT

pragma solidity 0.8.9;

import { AxelarExecutable } from '@axelar-network/axelar-gmp-sdk-solidity/contracts/executable/AxelarExecutable.sol';
import { IAxelarGateway } from '@axelar-network/axelar-gmp-sdk-solidity/contracts/interfaces/IAxelarGateway.sol';
import { IAxelarGasService } from '@axelar-network/axelar-gmp-sdk-solidity/contracts/interfaces/IAxelarGasService.sol';
import { StringToAddress, AddressToString } from '@axelar-network/axelar-gmp-sdk-solidity/contracts/utils/AddressString.sol';

contract GlueGateway is AxelarExecutable {
    using StringToAddress for string;
    using AddressToString for address;

    error NotEnoughValueForGas();
    event ContractCallSent(string destinationChain, string contractAddress, bytes payload, uint256 nonce);
    event FalseAcknowledgment(string destinationChain, string contractAddress, uint256 nonce);

    uint256 public nonce;
    mapping(uint256 => bool) public receivedAck;
    mapping(uint256 => bytes32) public destination;
    
    IAxelarGasService public immutable gasService;
    string public thisChain;

    // Proxy information
    string public proxyAdr;
    string public proxyChain;
    bool public isProxySet;

    constructor(
        address gateway_,
        address gasReceiver_,
        string memory thisChain_
    ) AxelarExecutable(gateway_) {
        gasService = IAxelarGasService(gasReceiver_);
        thisChain = thisChain_;
        
        isProxySet = false;
    }

    function setProxy(string memory proxyChain_, string memory proxyAdr_) external {
        proxyAdr = proxyAdr_;
        proxyChain = proxyChain_;
        isProxySet = true;
    }

    function _getDestinationHash(string memory destinationChain, string memory contractAddress)
            internal pure returns (bytes32) {
        return keccak256(abi.encode(destinationChain, contractAddress));
    }

    function forwardToProxy(
        bytes calldata payload
    ) external payable {
        assert(isProxySet);

        uint256 nonce_ = nonce;
        bytes memory modifiedPayload = abi.encode(nonce_, payload);

        if (msg.value == 0)  revert NotEnoughValueForGas();
        
        gasService.payNativeGasForContractCall{ value: msg.value }(
            address(this),
            proxyChain,
            proxyAdr,
            modifiedPayload,
            msg.sender
        );

        gateway.callContract(proxyChain, proxyAdr, modifiedPayload);
        emit ContractCallSent(proxyChain, proxyAdr, payload, nonce_);
        destination[nonce_] = _getDestinationHash(proxyChain, proxyAdr);
        nonce = nonce_ + 1;
    }

    function _execute(
        string calldata sourceChain,
        string calldata sourceAddress,
        bytes calldata payload
    ) internal override {
        uint256 nonce_ = abi.decode(payload, (uint256));
        if (destination[nonce_] != _getDestinationHash(sourceChain, sourceAddress)) {
            emit FalseAcknowledgment(sourceChain, sourceAddress, nonce_);
            return;
        }
        receivedAck[nonce_] = true;
        //get some gas back.
        destination[nonce_] = 0;
    }
}
